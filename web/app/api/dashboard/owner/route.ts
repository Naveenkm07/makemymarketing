...
export async function GET() {
  try {
    ...
    const activeScreens = screens?.filter((s: any) => s.availability).length || 0;
    ...
    if (screenIds.length > 0) {
      ...
      totalRevenue = bookings?.reduce((sum: number, b: any) => sum + (b.total_price || 0), 0) || 0;
    }
...
export async function POST(req: Request) {
  try {
    ...
    if (profileError || !profile || (profile as any).role !== "owner") {
